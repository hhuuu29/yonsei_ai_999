...
**DB 연결 완료·일반 사용자 로그인 설정 대기:** 운영 Supabase에 테이블과 접근 정책을 설치하고, 실제 Auth·REST로 저장/복원·계정 간 접근 차단·공유 생성/해제를 검증했다. 이메일 인증번호 발송에는 Custom SMTP가 필요하며, Google 로그인으로 전환하는 방안도 검토 중이다.

- 일정판의 **보관함에 저장** → 이메일 인증 → 저장할 내용 확인. 원문을 제외한 일정·할 일의 사본을 보관한다. 헤더 **보관함**에서 다시 열거나 삭제한다.
...
설치 SQL, 서버 환경변수, 이메일 OTP·SMTP 설정과 검증 방법은 [운영 연결 안내](docs/plans/2026-09-10-cloud-setup.md)를 참고한다.
